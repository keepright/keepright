...
fast_food, cinema, pharmacy, cafe, fuel
*/
...
query("
	...
	FROM _tmp_points_of_interest b
	WHERE NOT EXISTS( SELECT nt.node_id FROM node_tags nt WHERE nt.node_id=b.node_id AND nt.k='operator' )
", $db1);
...
query("
	...
	SELECT node_id, MIN(v) as v
	FROM node_tags
	WHERE k='amenity' AND v IN ('cinema', 'pharmacy', 'cafe', 'fast_food', 'fuel')
	GROUP BY node_id
", $db1);
query("
	INSERT INTO _tmp_errors(error_type, object_type, object_id, description, last_checked) 
