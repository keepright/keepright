...
query("
	...
	FROM node_tags
	WHERE k='amenity' AND v IN ('place_of_worship', 'cinema', 'pharmacy', 'cafe', 'fast_food', 'pub', 'restaurant', 'school', 'university', 'hospital', 'library', 'theatre', 'courthouse')
	GROUP BY node_id
", $db1);
query("
...
query("
	...
	SELECT node_id, MIN(v) as v
	FROM node_tags
	WHERE k='amenity' AND v IN ('bank')
	GROUP BY node_id
", $db1);
query("
	INSERT INTO _tmp_errors(error_type, object_type, object_id, description, last_checked) 
	SELECT $error_type, 'node', node_id, 'This node is tagged as ' || v || ' and therefore needs an operator tag', NOW()
	FROM _tmp_points_of_interest b
	WHERE NOT EXISTS( SELECT * FROM node_tags nt WHERE nt.node_id=b.node_id AND nt.k='operator' )
", $db1);
